...
### Ресурсы
Внутренние: Разработчики, тестировщики, Jira (баг-трекинг)
Сторонние:  Postman (API), Selenium (UI)
...
### Ресурсы:  
Внутренние: QA инженеры, разработчики, DevOps, Jira (баг-трекинг).
...
Необходимо держать связь с командой поддержки с целью анализа жалоб пользователей

### Патчи
Перед выкаткой в продакшен патчей необходимо проводить регрессионное/дымковое тестирование с целью выявления багов, ломающих функионал неизмененной части кода проекта. Также в тестах необходимы и новые части проекта, относящиеся к патчу (hotfix тесты).

### Ресурсы:  
Внутренние: Команда поддержки, разработчики, Jira (баг-трекинг)
Сторонние: Инструменты мониторинга (напр. Sentry).
     
### Метрики качества:  
1. Количество дефектов, найденных после релиза.
2. Уровень удовлетворенности пользователей программным продуктом.
